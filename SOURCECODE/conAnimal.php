          ...
          <?php 
					...
					if(isset($_POST['btnPesq'])){
						if($_POST['ibnome'] != NULL || $_POST['ibcod'] != NULL){
							...
							if($_POST['ibcod'] != NULL){
								if($and){
									$condicao .= " AND codigo_ani = ". $_POST['ibcod'] ;
								}else{
									$condicao .= " codigo_ani = ". $_POST['ibcod'] ;
								}
							}
						}
					}
					...
					$sql = "SELECT animal.codigo_ani, animal.nome, animal.idade, animal.raca, animal.categoria, animal.status, cliente.nome as nome_cli from animal left join cliente on animal.codigo_cli = cliente.codigo_cli " . $condicao;
         
					$res = mysqli_query($link,$sql);
					while ($dados = mysqli_fetch_array($res)){
						echo "<tr>";
						echo "<td>".$dados['codigo_ani']."</td>";
						...
						$id = base64_encode($dados['codigo_ani']);
						echo "<td><a href='frmAnimal.php?tela=editar&id=".$id."' title='Editar Animal'> <spam class='glyphicon glyphicon-pencil'> </spam> </a>";
						echo "<a href='frmAnimal.php?tela=excluir&id=".$id."' title='Excluir Animal'> <spam class='glyphicon glyphicon-remove'> </spam> </a>";
						echo "</td>";
						echo "</tr>";
					}
					desconectar($link);
